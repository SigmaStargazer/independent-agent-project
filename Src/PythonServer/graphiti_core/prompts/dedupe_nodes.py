...
from typing import Any, Protocol, TypedDict, List, Optional
...
class NodeDuplicate(BaseModel):
    ...
    duplicates: Optional[List[int]] = Field(
        default_factory=list, # ✅ 关键修改：即使 LLM 返回 null，也会变成 []
    ...
    @classmethod
    def ensure_duplicates_list(cls, v):
...
class NodeResolutions(BaseModel):
    ...
    # 关键：启用模型配置，确保嵌套模型被正确验证
    model_config = ConfigDict(
        validate_default=True,      # 验证默认值
        validate_assignment=True,  # 赋值时也验证
        extra='forbid'             # 禁止多余字段（可选，推荐）
    )
