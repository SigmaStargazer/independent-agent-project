...
# Guidelines:
# 1. Always try to extract an entity that the JSON represents. This will often be something like a "name" or "user field".
# 2. Do NOT extract any properties that contain dates.

# ### ✅ DO:
# - Return a JSON object with ONE field: `"extracted_entities"`
# - `"extracted_entities"` is a list of objects with:
#   - `"name"`: (string) the entity name (e.g., product name)
#   - `"entity_type_id"`: (integer) the matching type ID from ENTITY TYPES
# - Return ONLY the JSON. No markdown, no text, no explanations.

# ### ❌ DO NOT:
# - Return `$defs`, `properties`, `type`, or any schema-related fields
# - Return `"type": "object"` or `"required"` arrays
# - Return anything that looks like OpenAPI or JSON Schema
# - Return the word "schema"

# ### ✅ CORRECT EXAMPLE:
# {{"extracted_entities": [
#   {{"name": "Laptop X1", "entity_type_id": 1}},
#   {{"name": "Premium Plan", "entity_type_id": 3}}
# ]}}

# ### ❌ WRONG EXAMPLES:
# {{"$defs": {{"ExtractedEntity": ...}}}}  ← SCHEMA, NOT ALLOWED
# {{"type": "object", "properties": ...}} ← SCHEMA, NOT ALLOWED
# "Just a string" ← NOT JSON
# `{{"extracted_entities": ...}}` ← NO MARKDOWN CODE BLOCKS

# ## NOW PROCESS THE JSON AND RETURN ONLY THE ENTITY LIST:
# """
    return [
        Message(role='system', content=sys_prompt),
        Message(role='user', content=user_prompt),
