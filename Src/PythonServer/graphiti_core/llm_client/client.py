...
class LLMClient(ABC):
    ...
    async def generate_response(
        ...
        # === 核心：是否启用结构化输出 ===
        use_structured_output = response_model is not None
        schema = response_model.model_json_schema() if use_structured_output else None

        # === 调用底层 API，确保传递 response_format ===
        try:
            response = await self._generate_response_with_retry(
                messages=messages,
            ...
            # 如果 _generate_response_with_retry 不支持 response_format，先忽略
            print("⚠️ 注意：_generate_response_with_retry 不支持 response_format 参数")
            response = await self._generate_response_with_retry(
                messages=messages,
                max_tokens=max_tokens,
                model_size=model_size,
        ...
        try:
            ...
            raise ValueError(f"LLM 响应格式异常: {response}") from e

        # === 如果是结构化输出，解析并验证 ===
        if use_structured_output:
            try:
                data = json.loads(content)  # 先解析 JSON
            except json.JSONDecodeError as e:
                raise ValueError(f"LLM 返回的不是合法 JSON: {content}") from e

            # 检查是否返回的是 schema 而不是数据
            if '$defs' in data or data.get('type') == 'object':
                raise ValueError(
                    f"LLM 返回了 JSON Schema 而不是数据！请确保 prompt 不诱导返回 schema。\n"
                    f"返回内容: {content}"
                )
            ...
            try:
                ...
                return validated_model.model_dump()  # 返回字典
            except pydantic.ValidationError as ve:
                raise ValueError(f"数据不符合 {response_model.__name__} 结构: {ve}") from ve
        else:
