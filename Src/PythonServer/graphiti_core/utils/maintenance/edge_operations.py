    ...
    while facts_missed and reflexion_iterations <= MAX_REFLEXION_ITERATIONS:
        ...
        if isinstance(llm_response, dict):
            try:
                ...
                print(f"LLM 返回数据结构无效，无法构造 EdgeDuplicate: {ve}")
                edges_data = []
        else:
            raise ValueError(f"LLM 响应必须是字典，实际类型: {type(llm_response)}")
...
async def resolve_extracted_edge(
    ...
    # 从 llm_response 中提取 EdgeDuplicate 所需的字段
    if isinstance(llm_response, dict):
        # 如果有 'properties'，则从里面取
        if 'properties' in llm_response:
            data = llm_response['properties']
        else:
            # 否则直接使用整个字典
            data = llm_response
        ...
        # 现在用 data 创建对象
        try:
            response_object = EdgeDuplicate(**data)
        except pydantic.ValidationError as ve:
            raise ValueError(f"LLM 返回数据结构无效，无法构造 EdgeDuplicate: {ve}") from ve
    else:
        raise ValueError(f"LLM 响应必须是字典，实际类型: {type(llm_response)}")
