...
#def parse_db_date(input_date: neo4j_time.DateTime | str | None) -> datetime | None:
def parse_db_date(input_date: neo4j_time.DateTime | str | datetime | None) -> datetime | None:
    if isinstance(input_date, neo4j_time.DateTime):
        return input_date.to_native()
    ...
    # kuzu db returns datetime type object here, so we need an addtional logic below
