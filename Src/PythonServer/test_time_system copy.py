...
class UserAlarmManager:
    ...
    def add_alarm(self, user_id, hour, minute, repeat=False, description="无描述"):
        with self.lock:
            ...
            self.user_alarms.setdefault(user_id, {})[alarm_id] = {
                'time': (hour, minute),
                'repeat': repeat,
                'callbacks': [],
                'last_trigger': None,
    ...
    def check_alarms(self, current_time):
        with self.lock:
            for user_id, alarms in list(self.user_alarms.items()):
                for alarm_id, alarm in list(alarms.items()):
                    hour, minute = alarm['time']
                    last_trigger = alarm.get('last_trigger')
                    description = alarm.get('description', '无描述')  # 👈 获取描述字段
                    ...
                    if last_trigger is None or last_trigger < target_time:
                        ...
                        alarm['last_trigger'] = target_time
