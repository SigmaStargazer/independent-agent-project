...
namespace Network
{
    class ClientBase : MonoSingleton<ClientBase>
    {
        ...
        public void SendMessage(NetMessage message)
        {
            ...
            //byte[] messageBytes = PackageHandler.PackMessage(message); // 测试
            //Debug.Log($"Sending message: {message} (Length: {messageBytes.Length})"); // 测试

            byte[] messageBytes = PackageHandler.PackMessage(message); // 测试
            Debug.Log($"Sending message: {message} (Length: {messageBytes.Length})"); // 测试
            Debug.Log($"Message bytes: {BitConverter.ToString(messageBytes)}"); // 打印消息的字节内容
