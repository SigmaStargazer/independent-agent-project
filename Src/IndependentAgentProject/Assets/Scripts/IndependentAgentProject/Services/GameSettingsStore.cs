...
namespace Services
{
    ...
    public static class GameSettingsStore
    {
        ...
        /// <summary>默认显示模式（全屏）/ 默认分辨率下标（1920x1080）。仅在设置文件没有任何值时启用（v0.23.4）。</summary>
        private const int kDefaultMode = 2;   // FullScreenMode.ExclusiveFullScreen
        private const int kDefaultRes = 2;    // (1920, 1080)

        /// <summary>
        /// 读取画面设置。
        /// 返回 (hasValue, mode, res)：hasValue=false 表示文件不存在 / 为空 / 解析失败（即「没有任何值」），
        /// 此时应由调用方使用默认值（kDefaultMode/kDefaultRes，全屏 + 1920x1080）。
        /// </summary>
        public static (bool hasValue, int mode, int res) Load()
        {
            // 文件不存在 → 没有任何值，直接返回默认
            string path = Path.Combine(JsonConfigIO.ConfigDir(), FileName);
            if (!File.Exists(path))
            {
                return (false, kDefaultMode, kDefaultRes);
            }
            ...
            return (true, settings.displayModeIndex, settings.resolutionIndex);
        }
        ...
        public static void Save(int mode, int res)
        {
            ...
            Debug.Log($"[GameSettingsStore] 已保存画面设置到 {JsonConfigIO.ConfigDir()}/{FileName}（模式={mode} 分辨率下标={res}）");
        }
    }
}
