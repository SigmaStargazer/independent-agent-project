...
namespace IndependentAgentProject
{
    ...
    /// 修改经 Command（ChangeDisplayModeCommand / ChangeResolutionCommand / ChangeLanguageCommand），落盘经 SaveGameSettingsCommand。
    /// v0.23.5：新增 Language（语言），OnInit 同步初始化 UITextProvider 当前语言。
    /// </summary>
    public interface IGameSettingsModel : IModel
    {
        ...
        /// <summary>语言（0 简体中文 / 1 English）。v0.23.5 新增。</summary>
        BindableProperty<int> Language { get; }
    }
    ...
    public class GameSettingsModel : AbstractModel, IGameSettingsModel
    {
        ...
        public BindableProperty<int> Language { get; } = new BindableProperty<int>();

        protected override void OnInit()
        {
            var (hasValue, mode, res, language) = GameSettingsStore.Load();
            // 文件没有任何值时用默认值（全屏 + 1920x1080 + 简体中文，Store 已回填），有值时用文件值
            DisplayModeIndex.Value = mode;
            ResolutionIndex.Value = res;
            Language.Value = language;
            // 同步初始化文案表当前语言（v0.23.5）：Model 是语言唯一数据源，UI 启动即按保存语言取文案。
