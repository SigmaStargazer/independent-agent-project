...
| **`DevDocs/Architecture/`** | 跨版本的**架构梳理文档**（如生命周期架构、系统归属、协议分层等）；随架构演进同步更新，不随单个版本归档 |
| **`DevDocs/Analysis/`** | 临时/专题分析文档（实现方式疑问分析等），供你审阅后决定是否落入版本 |
...
3. **跨版本架构梳理文档**：放在 `DevDocs/Architecture/`，用于沉淀**不随单个版本归档**的架构原则与系统归属（如生命周期、协议分层、存储模型等）。与 `feature-design/`（功能设计原则）的区别：`Architecture/` 侧重**系统级架构与模块归属**，`feature-design/` 侧重**功能级设计原则**。例如：
...
架构类版本改动（如生命周期重构）应在 `solution.md` 中引用对应的 `Architecture/` 文档，并检查是否违反其中已确认的架构原则。
